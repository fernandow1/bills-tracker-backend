 ...
 */
module.exports = class Migrations1757178110011 {
    name = 'Migrations1757178110011'

    async up(queryRunner) {
        await queryRunner.query(`
            ALTER TABLE \`product\` DROP COLUMN \`net_price\`
        `);
        await queryRunner.query(`
            ALTER TABLE \`product\` DROP COLUMN \`net_unit\`
        `);
        await queryRunner.query(`
    ...
    async down(queryRunner) {
        ...
        await queryRunner.query(`
            ALTER TABLE \`product\`
            ADD \`quantity\` int UNSIGNED NOT NULL
        `);
        await queryRunner.query(`
            ALTER TABLE \`product\`
            ADD \`net_unit\` enum ('g', 'kg', 'l', 'ml', 'u') NOT NULL
        `);
        await queryRunner.query(`
            ALTER TABLE \`product\`
            ADD \`net_price\` decimal NOT NULL
        `);
    }
}
